...
6. Если `XUI_DOMAIN` задан и отличается от `webDomain` из БД, сначала пробуется домен из `.env`.
7. Если `XUI_DOMAIN` пустой, домен берётся только из `webDomain` в БД.
8. Если домена нет ни в `.env`, ни в `webDomain`, SSL-сертификат не выпускается.
9. Если enabled inbound с TLS ссылается на отсутствующий сертификат, путь автоматически заменяется на сертификат `XUI_DOMAIN`.
10. Если `xrayTemplateConfig` не существует, он создаётся из `config.json`.
11. Применяются настройки логов Xray через `jq`.
12. Панель стартует с применёнными настройками.
