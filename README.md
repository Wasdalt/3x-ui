...
5. Если `XUI_PORT` пустой и `webPort` в БД пустой, генерируется свободный порт и записывается только в БД.
6. Если `XUI_BASE_PATH` пустой и `webBasePath` в БД пустой, генерируется скрытый путь и записывается только в БД.
7. Если `XUI_DOMAIN` задан и отличается от `webDomain` из БД, сначала пробуется домен из `.env`.
8. Если `XUI_DOMAIN` пустой, домен берётся только из `webDomain` в БД.
9. Если домена нет ни в `.env`, ни в `webDomain`, SSL-сертификат не выпускается.
10. Если enabled inbound с TLS ссылается на отсутствующий сертификат, путь автоматически заменяется на сертификат `XUI_DOMAIN`.
11. Если `xrayTemplateConfig` не существует, он создаётся из `config.json`.
12. Применяются настройки логов Xray через `jq`.
13. Панель стартует с применёнными настройками.
